...
String FirebaseArduino::push(const String& path, const JsonVariant& value) {
  String buf;
  value.printTo(buf);
  auto push = FirebasePush(host_, auth_, path.c_str(), buf.c_str());
  error_ = push.error();
...
void FirebaseArduino::set(const String& path, const JsonVariant& value) {
  String buf;
  value.printTo(buf);
  auto set = FirebaseSet(host_, auth_, path.c_str(), buf.c_str());
  error_ = set.error();
}

FirebaseObject FirebaseArduino::get(const String& path) {
  auto get = FirebaseGet(host_, auth_, path.c_str());
  error_ = get.error();
  if (failed()) {
...
void FirebaseArduino::remove(const String& path) {
  auto remove = FirebaseRemove(host_, auth_, path.c_str());
  error_ = remove.error();
}

void FirebaseArduino::stream(const String& path) {
  auto stream = FirebaseStream(host_, auth_, path.c_str());
  error_ = stream.error();
}
